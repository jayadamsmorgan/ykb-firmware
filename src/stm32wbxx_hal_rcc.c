  ...
  FLASH.
    ...
    [..]
      ...
      (+) Configure the clock source(s) for peripherals which clocks are not
          derived from the System clock (SAI1, RTC, ADC, USB/RNG, USART1,
  LPUART1, LPTIMx, I2Cx, SMPS)
...
static HAL_StatusTypeDef RCC_SetFlashLatency(uint32_t Flash_ClkSrcFreq,
...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization
 ...
 external oscillators (HSE, HSI, LSE, MSI, LSI1, LSI2, PLL, CSS and MCO) and the
 System buses clocks (SYSCLK, HCLK1, HCLK2, HCLK4, PCLK1 and PCLK2).
 ...
 from 100KHZ to 48MHZ. It can be used to generate the clock for the USB FS (48
 MHz). The number of flash wait states is automatically adjusted when MSI range
 is updated with HAL_RCC_OscConfig() and the MSI is used as System clock source.

         (+) LSI1/LSI2 (low-speed internal): 32 KHz low consumption RC used as
 IWDG and/or RTC clock source.

         (+) HSE (high-speed external): 32 MHz crystal oscillator used directly
 or through the PLL as System clock source. Can be used also optionally as RTC
 clock source.

         (+) LSE (low-speed external): 32.768 KHz oscillator used optionally as
 RTC clock source or the RF system Auto-wakeup from Stop and Standby modes.
 ...
 (up to 64MHz).
           (++) The second output is used to generate the clock for the USB FS
 (48 MHz), the random analog generator (<=48 MHz)
 ...
 clock is automatically switched to MSI or the HSI oscillator (depending on the
             STOPWUCK configuration) and an interrupt is generated if enabled.
             The interrupt is linked to the CPU1 and CPU2 NMI (Non-Maskable
 Interrupt) exception vector.

         (+) LSECSS: once enabled, if a LSE clock failure occurs, the LSE
             clock is no longer supplied to the RTC but no hardware action is
 made to the registers. If the MSI was in PLL-mode, this mode is disabled. In
 Standby mode a wakeup is generated. In other modes an interrupt can be sent to
 wakeup the software

         (+) MCO (microcontroller clock output): used to output MSI, LSI1, LSI2,
 HSI, LSE, HSE (before and after stabilization), SYSCLK, HSI48 or main PLL clock
 (through a configurable prescaler) on PA8, PB6 & PA15 pins.

    [..] System, AHB and APB buses clocks configuration
         (+) Several clock sources can be used to drive the System clock
 (SYSCLK): MSI, HSI, HSE and main PLL. The AHB clock (HCLK1) is derived from
 ...
 these clocks. The AHB4 clock (HCLK4) is derived from System clock through
 configurable prescaler and used to clock the FLASH

         -@- All the peripheral clocks are derived from the System clock
 (SYSCLK) except:

           (+@) SAI: the SAI clock can be derived either from a specific PLL
 (PLLSAI1) or (PLLSYS) or from an external clock mapped on the SAI_CKIN pin. You
 have to use HAL_RCCEx_PeriphCLKConfig() function to configure this clock.
           (+@) RTC: the RTC clock can be derived either from the LSI, LSE or
 HSE clock divided by 32. You have to use __HAL_RCC_RTC_ENABLE() and
 ...
 is 64 MHz. The maximum frequency of the HCLK2 is 32 MHz. The clock source
 frequency should be adapted depending on the device voltage range as listed in
 the Reference Manual "Clock source frequency versus voltage scaling" chapter.
...
HAL_StatusTypeDef HAL_RCC_DeInit(void) {
...
#if defined(SAI1)
    CLEAR_BIT(RCC->CR, RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS |
                           RCC_CR_HSEON | RCC_CR_HSEPRE | RCC_CR_PLLON |
                           RCC_CR_PLLSAI1ON);
#else
    CLEAR_BIT(RCC->CR, RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS |
                           RCC_CR_HSEON | RCC_CR_HSEPRE | RCC_CR_PLLON);
    ...
    WRITE_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLR_0 | RCC_PLLCFGR_PLLQ_0 |
    ...
    WRITE_REG(RCC->PLLSAI1CFGR,
              RCC_PLLSAI1CFGR_PLLR_0 | RCC_PLLSAI1CFGR_PLLQ_0 |
 ...
 * @param  RCC_OscInitStruct  pointer to a @ref RCC_OscInitTypeDef structure
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
        ...
        if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_MSI) ||
             ...
             */
                ...
                /* To correctly read data from FLASH memory, the number of wait
                   states (LATENCY) must be correctly programmed according to
                   the frequency of the AHB4 clock and the supply voltage of the
                   device. */
    ...
    if ((((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI1) ==
        ...
        if (RCC_OscInitStruct->LSIState != RCC_LSI_OFF) {
            /*------------------------------ LSI2 selected by default (when
             * Switch ON) -------------------------*/
            if (((RCC_OscInitStruct->OscillatorType) &
                 RCC_OSCILLATORTYPE_LSI2) == RCC_OSCILLATORTYPE_LSI2) {
                assert_param(IS_RCC_LSI2_CALIBRATION_VALUE(
                    RCC_OscInitStruct->LSI2CalibrationValue));
                ...
                /* 2. Enable the Internal Low Speed oscillator (LSI2) and set
                 * trimming value */
                ...
                /* Adjusts the Internal Low Spee oscillator (LSI2) calibration
                 * value */
    ...
    if (RCC_OscInitStruct->PLL.PLLState != RCC_PLL_NONE) {
        ...
        if (RCC_OscInitStruct->PLL.PLLState == RCC_PLL_ON) {
            ...
            if ((READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLSRC) !=
                ...
                (READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLM) !=
                 RCC_OscInitStruct->PLL.PLLM) ||
                ((READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLN) >>
                  RCC_PLLCFGR_PLLN_Pos) != RCC_OscInitStruct->PLL.PLLN) ||
                (READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLP) !=
                 RCC_OscInitStruct->PLL.PLLP) ||
                (READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLQ) !=
                 RCC_OscInitStruct->PLL.PLLQ) ||
                (READ_BIT(temp_pllconfig, RCC_PLLCFGR_PLLR) !=
            ...
            if (temp_sysclksrc != RCC_SYSCLKSOURCE_STATUS_PLLCLK) {
                ...
                /* Disable the PLL source and outputs to save power when PLL is
                 * off */
#if defined(SAI1) && defined(USB)
                CLEAR_BIT(RCC->PLLCFGR,
                          (RCC_PLLCFGR_PLLSRC | RCC_PLLCFGR_PLLPEN |
                           RCC_PLLCFGR_PLLQEN | RCC_PLLCFGR_PLLREN));
#else
                CLEAR_BIT(RCC->PLLCFGR,
                          (RCC_PLLCFGR_PLLSRC | RCC_PLLCFGR_PLLREN));
#endif /* SAI1 && USB */
            } else {
                /* PLL is already used as System core clock */
                return HAL_ERROR;
 ...
 * Reset, the MSI frequency is set to its default value 4 MHz.
 *
 ...
 * correctly HPRE[3:0] bits to ensure that HCLK1 not exceed the maximum allowed
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
    ...
    /* To correctly read data from FLASH memory, the number of wait states
      ...
      FLASH clock (HCLK4) and the supply voltage of the device. */
    ...
    /*-------------------------- HCLK2 Configuration
     * --------------------------*/
    if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK2) ==
        RCC_CLOCKTYPE_HCLK2) {
    ...
    /*-------------------------- HCLK4 Configuration
     * --------------------------*/
    if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK4) ==
        RCC_CLOCKTYPE_HCLK4) {
...
/**
 * @brief  Select the clock source to output on MCO1 pin(PA8) or MCO2 pin (PB6)
 * or MCO3 pin (PA15).
 ...
 *            @arg @ref RCC_MCO1SOURCE_HSE_BEFORE_STAB  HSE clock before
 * stabilization selected as MCO source
 ...
 * factors.
 * @note     (*) HSI_VALUE is a constant defined in stm32wbxx_hal_conf.h file
 * (default value 16 MHz) but the real value may vary depending on the
 * variations in voltage and temperature.
 * @note     (**) HSE_VALUE is a constant defined in stm32wbxx_hal_conf.h file
...
uint32_t HAL_RCC_GetHCLKFreq(void) {
    ...
    return ((uint32_t)(__LL_RCC_CALC_HCLK1_FREQ(HAL_RCC_GetSysClockFreq(),
...
uint32_t HAL_RCC_GetHCLK4Freq(void) {
    ...
    return ((uint32_t)(__LL_RCC_CALC_HCLK4_FREQ(HAL_RCC_GetSysClockFreq(),
...
uint32_t HAL_RCC_GetPCLK2Freq(void) {
    ...
    return ((uint32_t)(__LL_RCC_CALC_PCLK2_FREQ(HAL_RCC_GetHCLKFreq(),
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
    ...
    regICSRvalue =
        RCC->ICSCR; /* Get Internal Clock Sources Calibration register */
    ...
    RCC_OscInitStruct->PLL.PLLState =
        ((regvalue & RCC_CR_PLLON) >> RCC_CR_PLLON_Pos) + 1U;
    ...
    RCC_OscInitStruct->PLL.PLLN =
        ((regPLLCFGRvalue & RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos);
    ...
    RCC_OscInitStruct->LSIState =
        ((regvalue & RCC_LSI_ON) > 0U) ? RCC_LSI_ON : 0U;
 ...
 * @retval None
 */
void HAL_RCC_EnableCSS(void) { LL_RCC_HSE_EnableCSS(); }
...
/**
  ...
  * @param  MSI_Range  MSI range value from @ref RCC_MSIRANGE_0 to @ref
  RCC_MSIRANGE_11
  * @retval HAL status
  */
static HAL_StatusTypeDef RCC_SetFlashLatencyFromMSIRange(uint32_t MSI_Range) {
    ...
    flash_clksrcfreq =
        __LL_RCC_CALC_HCLK4_FREQ(msifreq, LL_RCC_GetAHB4Prescaler());

#if defined(PWR_CR1_VOS)
    return RCC_SetFlashLatency((flash_clksrcfreq / MEGA_HZ),
 ...
 * @param  VCORE_Voltage     Current Vcore voltage (PWR_REGULATOR_VOLTAGE_SCALE1
 * or PWR_REGULATOR_VOLTAGE_SCALE2)
 * @retval HAL status
 */
static HAL_StatusTypeDef RCC_SetFlashLatency(uint32_t Flash_ClkSrcFreq,
                                             uint32_t VCORE_Voltage) {
...
#endif /* PWR_CR1_VOS */
    /* Flash Latency range */
    const uint32_t FLASH_LATENCY_RANGE[] = {FLASH_LATENCY_0, FLASH_LATENCY_1,
                                            FLASH_LATENCY_2, FLASH_LATENCY_3};
    uint32_t latency = FLASH_LATENCY_0; /* default value 0WS */
    uint32_t tickstart;

#if defined(PWR_CR1_VOS)
    if (VCORE_Voltage == PWR_REGULATOR_VOLTAGE_SCALE1) {
        for (uint32_t index = 0; index < __COUNTOF(FLASH_CLK_SRC_RANGE_VOS1);
             index++) {
    ...
    {
        for (uint32_t index = 0; index < __COUNTOF(FLASH_CLK_SRC_RANGE_VOS2);
             index++) {
