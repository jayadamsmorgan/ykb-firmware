...
void dma_init() {
    ...
    //     ;
    //
    // DMA1 DMA1_Stream5->CR |= DMA_CHANNEL_3;
...
void updateStrip(LedStrip *strip) {
    ...
    //
    //     color = (strip->led[i].g << 16) | (strip->led[i].r << 8) |
    //             (strip->led[i].b);
    //
    //     for (int j = 23; j >= 0; --j) {
    //         if (color & (1 << j))
    //             buffer[23 - j] = LOGIC_1_TICKS;
    //         else
    //             buffer[23 - j] = LOGIC_0_TICKS;
    //     }
    //
...
Led setHSB(int hue, uint8_t sat, uint8_t bright) {
    ...
    Led newColor = {(uint8_t)(r * 255), (uint8_t)(g * 255), (uint8_t)(b * 255)};
