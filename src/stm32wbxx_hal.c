...
#define __STM32WBxx_HAL_VERSION                                                \
    ((__STM32WBxx_HAL_VERSION_MAIN << 24U) |                                   \
 ...
 * each 1 millisecond, which is clocked by the MSI (at this stage, the clock is
 * not yet configured and thus the system is running from the internal MSI at 4
 * MHz). Set NVIC Group Priority to 4. Calls the HAL_MspInit() callback function
 * defined in user file "stm32wbxx_hal_msp.c" to do the global low level
...
__weak HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
    ...
    if ((uint32_t)uwTickFreq != 0U) {
        ...
        if (HAL_SYSTICK_Config(HAL_RCC_GetHCLKFreq() /
 ...
 */
uint32_t HAL_GetHalVersion(void) { return __STM32WBxx_HAL_VERSION; }

/**
 * @brief  Returns the device revision identifier.
 * @retval Device revision identifier
 */
uint32_t HAL_GetREVID(void) { return (LL_DBGMCU_GetRevisionID()); }

/**
 * @brief  Returns the device identifier.
 * @retval Device identifier
 */
uint32_t HAL_GetDEVID(void) { return (LL_DBGMCU_GetDeviceID()); }
 ...
 * @retval None
 */
void HAL_DBGMCU_EnableDBGSleepMode(void) { LL_DBGMCU_EnableDBGSleepMode(); }

/**
 * @brief  Disable the Debug Module during SLEEP mode
 * @retval None
 */
void HAL_DBGMCU_DisableDBGSleepMode(void) { LL_DBGMCU_DisableDBGSleepMode(); }

/**
 * @brief  Enable the Debug Module during STOP mode
 * @retval None
 */
void HAL_DBGMCU_EnableDBGStopMode(void) { LL_DBGMCU_EnableDBGStopMode(); }

/**
 * @brief  Disable the Debug Module during STOP mode
 * @retval None
 */
void HAL_DBGMCU_DisableDBGStopMode(void) { LL_DBGMCU_DisableDBGStopMode(); }

/**
 * @brief  Enable the Debug Module during STANDBY mode
 * @retval None
 */
void HAL_DBGMCU_EnableDBGStandbyMode(void) { LL_DBGMCU_EnableDBGStandbyMode(); }
 ...
 *         and will only be reset by a Hardware reset, including a reset after
 * Standby.)
 * @note Firmware writing 0 has no effect.
 * @retval None
 */
void HAL_SYSCFG_DisableSRAMFetch(void) { LL_SYSCFG_DisableSRAMFetch(); }
 ...
 *            @arg @ref SYSCFG_VREFBUF_VOLTAGE_SCALE0 : VREF_OUT1 around 2.048
 * V. This requires VDDA equal to or higher than 2.4 V.
 ...
 *          This parameter can be one of the following values:
 *            @arg @ref SYSCFG_VREFBUF_HIGH_IMPEDANCE_DISABLE : VREF+ pin is
 * internally connect to VREFINT output.
 *            @arg @ref SYSCFG_VREFBUF_HIGH_IMPEDANCE_ENABLE : VREF+ pin is high
 * impedance.
 ...
 * @note  Each VrefBuf voltage scale is calibrated in production for each
 * device, data stored in flash memory. Function @ref
 * HAL_SYSCFG_VREFBUF_VoltageScalingConfig retrieves and applies this
 * calibration data as trimming value at each scale change. Therefore,
 * optionally, function @ref HAL_SYSCFG_VREFBUF_TrimmingConfig can be used in a
 * second time to fine tune the trimming.
 ...
 * @retval None
 */
void HAL_SYSCFG_DisableVREFBUF(void) { LL_VREFBUF_Disable(); }
#endif /* VREFBUF */
 ...
 * @retval None
 */
void HAL_SYSCFG_EnableIOBooster(void) { LL_SYSCFG_EnableAnalogBooster(); }
 ...
 * @retval None
 */
void HAL_SYSCFG_DisableIOBooster(void) { LL_SYSCFG_DisableAnalogBooster(); }
 ...
 * @retval None
 */
void HAL_SYSCFG_EnableIOVdd(void) { LL_SYSCFG_EnableAnalogGpioSwitch(); }
 ...
 * @retval None
 */
void HAL_SYSCFG_DisableIOVdd(void) { LL_SYSCFG_DisableAnalogGpioSwitch(); }
#endif /* SYSCFG_CFGR1_ANASWVDD */

/**
 * @brief  Enable the access for security IP
 * @note   When the system is secure (ESE = 1), this register provides write
 * access security and can only be written by the CPU2. A write access from the
 * CPU1 will be ignored and a bus error is generated.
