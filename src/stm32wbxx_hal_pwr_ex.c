...
#define PWR_PORTE_AVAILABLE_PINS                                               \
    (PWR_GPIO_BIT_4 | PWR_GPIO_BIT_3 | PWR_GPIO_BIT_2 | PWR_GPIO_BIT_1 |       \
     PWR_GPIO_BIT_0)
#define PWR_PORTH_AVAILABLE_PINS                                               \
    (PWR_GPIO_BIT_3 | PWR_GPIO_BIT_1 | PWR_GPIO_BIT_0)

/** @defgroup PWREx_TimeOut_Value PWR Extended Flag Setting Time Out Value
 * @{
 */
#define PWR_FLAG_SETTING_DELAY_US                                              \
    50U /*!< Time out value for REGLPF and VOSF flags setting */
/**
 * @}
 */
 ...
 * @retval VOS bit field (PWR_REGULATOR_VOLTAGE_RANGE1 or
 * PWR_REGULATOR_VOLTAGE_RANGE2)
 */
uint32_t HAL_PWREx_GetVoltageRange(void) { return (PWR->CR1 & PWR_CR1_VOS); }
 ...
 * @retval None
 */
void HAL_PWREx_EnableRFPhaseIT(void) { SET_BIT(PWR->CR3, PWR_CR3_ECRPE_Msk); }
 ...
 * @retval None
 */
void HAL_PWREx_EnableBLEActivityIT(void) { SET_BIT(PWR->CR3, PWR_CR3_EBLEA); }
 ...
 * @retval None
 */
void HAL_PWREx_Enable802ActivityIT(void) { SET_BIT(PWR->CR3, PWR_CR3_E802A); }
 ...
 * @retval None
 */
void HAL_PWREx_EnableHOLDC2IT(void) { SET_BIT(PWR->CR3, PWR_CR3_EC2H); }

/**
 * @brief Disable CPU2 on-Hold interrupt.
 * @retval None
 */
void HAL_PWREx_DisableHOLDC2IT(void) { CLEAR_BIT(PWR->CR3, PWR_CR3_EC2H); }
 ...
 * PWR_GPIO_H to select the GPIO peripheral.
 ...
 * PWR_GPIO_A..PWR_GPIO_H to select the GPIO peripheral.
 * @param GPIONumber Specify the I/O pins numbers.
 *         This parameter can be one of the following values:
 *         PWR_GPIO_BIT_0, ..., PWR_GPIO_BIT_15 (except for PORTH where less
 *         I/O pins are available) or the logical OR of several of them to reset
 ...
 *         @arg @ref PWR_BOR_SMPS_FORCE_BYPASS
 */
uint32_t HAL_PWREx_GetBORConfig(void) { return LL_PWR_GetBORConfig(); }
#endif /* PWR_CR5_SMPSEN */

/****************************************************************************/
/**
 * @brief  Hold the CPU and their allocated peripherals after reset or wakeup
 * from stop or standby.
...
/**
 * @brief  Release Cortex CPU2 and allocated peripherals after reset or wakeup
 * from stop or standby.
 ...
 * @note   On devices STM32WB15xx, STM32WB10xx, STM32WB1Mxx retention is
 * extended to SRAM1, SRAM2a and SRAM2b.
 * @retval None
 */
void HAL_PWREx_EnableSRAMRetention(void) { LL_PWR_EnableSRAM2Retention(); }
 ...
 * @retval None
 */
void HAL_PWREx_DisableSRAMRetention(void) { LL_PWR_DisableSRAM2Retention(); }
...
HAL_StatusTypeDef HAL_PWREx_ConfigSMPS(PWR_SMPSTypeDef *sConfigSMPS) {
    ...
    __IO const uint32_t OutputVoltageLevel_calibration =
        (((*SMPS_VOLTAGE_CAL_ADDR) & SMPS_VOLTAGE_CAL) >>
         SMPS_VOLTAGE_CAL_POS); /* SMPS output voltage level calibrated in
                                   production */
    int32_t TrimmingSteps; /* Trimming steps between theoretical output voltage
                              and calibrated output voltage */
    int32_t OutputVoltageLevelTrimmed; /* SMPS output voltage level after
                                          calibration: trimming value added to
                                          required level */

    if (OutputVoltageLevel_calibration == 0UL) {
        /* Device with SMPS output voltage not calibrated in production: Apply
         * output voltage value directly */

        /* Update register */
        MODIFY_REG(PWR->CR5, PWR_CR5_SMPSVOS,
                   (sConfigSMPS->StartupCurrent | sConfigSMPS->OutputVoltage));
    } else {
        /* Device with SMPS output voltage calibrated in production: Apply
         * output voltage value after correction by calibration value */
        ...
        OutputVoltageLevelTrimmed =
            ((int32_t)((uint32_t)(sConfigSMPS->OutputVoltage >>
                                  PWR_CR5_SMPSVOS_Pos)) +
        ...
        MODIFY_REG(PWR->CR5, (PWR_CR5_SMPSSC | PWR_CR5_SMPSVOS),
                   (sConfigSMPS->StartupCurrent |
                    ((uint32_t)OutputVoltageLevelTrimmed)));
    }
 ...
 *
 *         (1) SMPS operating mode step down or open depends on system low-power
 * mode:
 *              - step down mode if system low power mode is run, LP run or
 * stop,
 *              - open mode if system low power mode is Stop1, Stop2, Standby or
 * Shutdown
 * @retval None
 */
void HAL_PWREx_SMPS_SetMode(uint32_t OperatingMode) {
    MODIFY_REG(PWR->CR5, PWR_CR5_SMPSEN,
               (OperatingMode & PWR_SR2_SMPSF)
                   << (PWR_CR5_SMPSEN_Pos - PWR_SR2_SMPSF_Pos));
}
 ...
 * @retval None
 */
void HAL_PWREx_EnableWakeUpPin(uint32_t WakeUpPinPolarity,
                               uint32_t wakeupTarget) {
 ...
 * optionally configure the Flash in power-down mode in setting the RUN_PD bit
 ...
 * @note  Case of Stop0 mode with SMPS: Before entering Stop 0 mode with SMPS
 * Step Down converter enabled, the HSI16 must be kept on by enabling HSI kernel
 * clock (set HSIKERON register bit).
 ...
 * peripheral requesting it. SRAM1, SRAM2 and register contents are preserved.
 *        The BOR is available.
 *        The voltage regulator is set in low-power mode but LPR bit must be
 * cleared to enter stop 2 mode. Otherwise, Stop 1 mode is entered.
 * @note  When exiting Stop 2 mode by issuing an interrupt or a wakeup event,
 *         the HSI RC oscillator is selected as system clock if STOPWUCK bit in
 * RCC_CFGR register is set; the MSI oscillator is selected if STOPWUCK is
 * cleared.
 * @note  Case of Stop2 mode and debugger probe attached: a workaround should be
 * applied. Issue specified in "ES0394 - STM32WB55Cx/Rx/Vx device errata": 2.2.9
 * Incomplete Stop 2 mode entry after a wakeup from debug upon EXTI line 48
 * event "With the JTAG debugger enabled on GPIO pins and after a wakeup from
 * debug triggered by an event on EXTI line 48 (CDBGPWRUPREQ), the device may
 * enter in a state in which attempts to enter Stop 2 mode are not fully
 *        effective ..."
 ...
 * domain is powered off. SRAM1, SRAM2, BKRAM and registers contents are lost
 * except for registers in the Backup domain. The BOR is not available.
